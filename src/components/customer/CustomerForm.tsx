...
const CustomerForm = ({ id, initialData }: any) => {
  ...
  const {
    ...
    formState: { errors },
    reset,
    setValue, // Add setValue from react-hook-form to update the form value
  } = useForm<FormData>({
    resolver: zodResolver(Schema),
    defaultValues: initialData || {},
  });

  // Dropdown options for customer type
  ...
  const handleCustomerTypeChange = (value: string) => {
    setCustomerType(value); // Update state
    setValue('customerType', value); // Update form value
  };

  useEffect(() => {
    if (initialData) {
      reset(initialData);
      setCustomerType(initialData.customerType || ''); // Set initial customer type
    }
  }, [initialData, reset]);

  const onSubmit = async (data: FormData) => {
    try {
      // Here you would call your API to create or update the customer
      toast(id ? 'Customer Updated Successfully' : 'Customer Created Successfully', { type: 'success' });
      reset();
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid grid-cols-2 gap-4 p-4">
          ...
          <CustomInputDropdown
            ...
            onChange={(value) => handleCustomerTypeChange(value)}
        ...
        <div>
          ...
          <div className="p-5 ml-7 bg-white shadow-md rounded-lg mx-auto">
            <textarea
              ...
              placeholder="Any additional notes or comments about the customer work details"
              value={text}
