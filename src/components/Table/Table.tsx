...
interface DataTableProps<TData, TValue> {
  ...
  hide?: boolean; // optional hide search bar
  link: string; // create link
  searchName?: string; // name of the column for search
  setPageIndex: React.Dispatch<React.SetStateAction<number>>; // function to set page index
  pageIndex: number; // current page index
  setPageSize: React.Dispatch<React.SetStateAction<number>>; // function to set page size
...
export function DataTable<TData, TValue>({
  ...
  return (
    <>
      <div className="flex justify-between px-5 py-4 mt-6 rounded-lg bg-white shadow-md">
        <div className="flex items-center space-x-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="gap-2 text-blue-600 hover:text-white bg-white hover:bg-blue-600 border-2 border-blue-600 rounded-full px-4 py-2 text-sm font-medium transition-all duration-200">
                <FaFilter />
                Columns
              </Button>
            </DropdownMenuTrigger>
        ...
        {hide && (
          <div className="relative max-w-sm w-full">
            ...
            />
            <MdManageSearch className="absolute top-1/2 left-3 transform -translate-y-1/2 text-gray-500" size={20} />
          </div>
        )}
      </div>

      <div className="overflow-x-auto mt-6 rounded-lg bg-white shadow-md">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className={`text-gray-500 text-sm font-semibold bg-gray-100 cursor-pointer ${header.column.getIsSorted()  ? 'bg-blue-100' : ''}`}
              ...
              <span className="text-gray-400"><FaFilter size={12} /></span>
            )}
      ...
      {/* Pagination Controls */}
      <div className="flex items-center justify-between py-4 mt-4 px-5">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="px-3 py-1 bg-blue-600 text-white rounded-full">{pageIndex + 1}</span>
          ...
          <span className="ml-2 text-gray-500 text-sm">Page {`${pageIndex + 1} of ${Math.ceil(data.length / pageSize)}`}</span>
        </div>
      </div>
    </>
  );
