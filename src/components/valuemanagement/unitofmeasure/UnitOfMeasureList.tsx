...
import { getAllUnitOfMeasures, deleteUnitOfMeasure } from '@/apis/unitofmeasure';
import { columns, UnitOfMeasureType } from '@/components/valuemanagement/unitofmeasure/columns';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const UnitOfMeasureList = () => {
  const [unitOfMeasures, setUnitOfMeasures] = React.useState<UnitOfMeasureType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedUnitOfMeasure, setSelectedUnitOfMeasure] = React.useState<UnitOfMeasureType | null>(null);
  ...
  const fetchUnitOfMeasures = async () => {
    try {
      setLoading(true);
      const response = await getAllUnitOfMeasures(pageIndex === 0 ? 1 : pageIndex, pageSize);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteUnitOfMeasure(deleteId);
      setUnitOfMeasures((prev) => prev.filter((item) => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
  ...
  const handleViewOpen = (listId: string) => {
    const item = unitOfMeasures.find((item) => item.listid === listId);
    setSelectedUnitOfMeasure(item || null);
    setOpenView(true);
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedUnitOfMeasure.descriptions}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedUnitOfMeasure.subDescription}
                      </div>
                    </div>
                  </div>
                </div>
