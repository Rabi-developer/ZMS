...
import { getAllDeliveryTerms, deleteDeliveryTerm } from '@/apis/deliveryterm';
import { columns, DeliveryTermType } from '@/components/valuemanagement/deliveryterm/columns';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const DeliveryTermList = () => {
  const [deliveryTerms, setDeliveryTerms] = React.useState<DeliveryTermType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedDeliveryTerm, setSelectedDeliveryTerm] = React.useState<DeliveryTermType | null>(null);
  ...
  const fetchDeliveryTerms = async () => {
    try {
      setLoading(true);
      const response = await getAllDeliveryTerms(pageIndex === 0 ? 1 : pageIndex, pageSize);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDeliveryTerm(deleteId);
      setDeliveryTerms((prev) => prev.filter((item) => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success('Deleted Successfully');
    } catch (error) {
      console.error('Failed to delete Delivery Term:', error);
      toast.error('Failed to delete Delivery Term');
    }
  };
  ...
  const handleViewOpen = (termId: string) => {
    const item = deliveryTerms.find((item) => item.listid === termId);
    setSelectedDeliveryTerm(item || null);
    setOpenView(true);
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedDeliveryTerm.descriptions}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
...
export default DeliveryTermList;