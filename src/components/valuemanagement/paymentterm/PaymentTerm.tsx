...
import { createPaymentTerm, updatePaymentTerm, getAllPaymentTerms } from '@/apis/paymentterm';
...
const PaymentTerm = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  React.useEffect(() => {
    if (isEdit) {
      const fetchPaymentTerm = async () => {
        const listid = window.location.pathname.split('/paymentterm/').pop();
        if (listid) {
          try {
            const response = await getAllPaymentTerms();
            const foundPaymentTerm = response.data.find((item: any) => item.listid === listid);
            if (foundPaymentTerm) {
  ...
  const onSubmit = async (data: PaymentTermData) => {
    ...
    try {
      ...
      toast.error('An error occurred while saving the Payment Term');
    }
  };
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? 'EDIT PAYMENT TERM' : 'ADD NEW PAYMENT TERM'}
        </h1>
      </div>
