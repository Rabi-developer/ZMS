...
import { getAllPaymentTerms, deletePaymentTerm } from '@/apis/paymentterm';
import { columns, PaymentTermType } from '@/components/valuemanagement/paymentterm/columns';
...
const PaymentTermList = () => {
  ...
  const [selectedPaymentTerm, setSelectedPaymentTerm] = React.useState<PaymentTermType | null>(null);
  ...
  const fetchPaymentTerms = async () => {
    try {
      setLoading(true);
      const response = await getAllPaymentTerms(pageIndex === 0 ? 1 : pageIndex, pageSize);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePaymentTerm(deleteId);
      setPaymentTerms((prev) => prev.filter((item) => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
  ...
  const handleViewOpen = (listId: string) => {
    const item = paymentTerms.find((item) => item.listid === listId);
    setSelectedPaymentTerm(item || null);
    setOpenView(true);
  };
