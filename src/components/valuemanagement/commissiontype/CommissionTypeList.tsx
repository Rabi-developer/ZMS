...
import { getAllCommissionTypes, deleteCommissionType } from '@/apis/commissiontype';
import { columns, CommissionTypeType } from '@/components/valuemanagement/commissiontype/colums';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const CommissionTypeList = () => {
  const [commissionTypes, setCommissionTypes] = React.useState<CommissionTypeType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedCommissionType, setSelectedCommissionType] = React.useState<CommissionTypeType | null>(null);
  ...
  const fetchCommissionTypes = async () => {
    try {
      setLoading(true);
      const response = await getAllCommissionTypes(pageIndex === 0 ? 1 : pageIndex, pageSize);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteCommissionType(deleteId);
      setCommissionTypes((prev) => prev.filter((item) => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success('Deleted Successfully');
    } catch (error) {
      console.error('Failed to delete Commission Type:', error);
      toast.error('Failed to delete Commission Type');
    }
  };
  ...
  const handleViewOpen = (commissionId: string) => {
    const item = commissionTypes.find((item) => item.listid === commissionId);
    setSelectedCommissionType(item || null);
    setOpenView(true);
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedCommissionType.descriptions}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedCommissionType.segment}
                      </div>
                    </div>
                  </div>
                </div>
