...
import { createAblAssests, getAllAblAssests, updateAblAssests, deleteAblAssests } from '@/apis/ablAssests';
...
//  Save highlightedItems to localStorage whenever it changes
  ...
  const fetchAblAssests = async () => {
    try {
      setLoading(true);
      const response: ApiResponse<Account[]> = await getAllAblAssests(pageIndex === 0 ? 1 : pageIndex, pageSize);
  ...
  useEffect(() => {
    const initialAccountExists = accounts.some(account => account.listid === '3');
    ...
    try {
      ...
      if (editingId) {
        ...
        if (accountToUpdate) {
          ...
          response = await updateAblAssests(editingId, updateData);
        ...
        response = await createAblAssests(newAccount);
    ...
    if (action === 'add') {
      ...
      try {
        ...
        if (account) {
          ...
          await updateAblAssests(id, updatedAccount);
  ...
  const sortAccountsByListId = (accountsToSort: Account[]): Account[] => {
    return [...accountsToSort].sort((a, b) => {
      const aListId = a.listid || '';
      const bListId = b.listid || '';
      const aParts = aListId.split('.').map(part => parseInt(part) || 0);
      const bParts = bListId.split('.').map(part => parseInt(part) || 0);
      ...
      <div className="p-4 border-2 border-[#3a614c] shadow-lg rounded-lg bg-white dark:bg-[#1a2b21]">
        <div className="w-full bg-[#3a614c] h-[7vh] rounded-lg dark:bg-[#2e4c3d] mb-4 pt-2">
          <h1 className="text-[24px] font-mono ml-10 pt-2 text-white flex gap-2">
            ...
            <span className="">LIST OF ACCOUNT-Assests</span>
          </h1>
        </div>
