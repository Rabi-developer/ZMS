...
import { createAblRevenue, getAllAblRevenue, updateAblRevenue, deleteAblRevenue } from '@/apis/ablRevenue';
...
//  Save highlightedItems to localStorage whenever it changes
  ...
  const fetchAblRevenue = async () => {
    try {
      setLoading(true);
      const response: ApiResponse<Account[]> = await getAllAblRevenue(pageIndex === 0 ? 1 : pageIndex, pageSize);
  ...
  useEffect(() => {
    const initialAccountExists = accounts.some(account => account.listid === '5');
    ...
    try {
      ...
      if (editingId) {
        ...
        if (accountToUpdate) {
          ...
          response = await updateAblRevenue(editingId, updateData);
        ...
        response = await createAblRevenue(newAccount);
    ...
    if (action === 'add') {
      ...
      try {
        ...
        if (account) {
          ...
          await updateAblRevenue(id, updatedAccount);
      ...
      <div className="p-4 border-2 border-[#3a614c] shadow-lg rounded-lg bg-white dark:bg-[#1a2b21]">
        <div className="w-full bg-[#3a614c] h-[7vh] rounded-lg dark:bg-[#2e4c3d] mb-4 pt-2">
          <h1 className="text-[24px] font-mono ml-10 pt-2 text-white flex gap-2">
            ...
            <span className="">LIST OF ACCOUNT-Revenue</span>
          </h1>
        </div>
