...
export const columns = (
  ...
  },
  {
    ...
    cell: ({ row }: { row: Row<Receipt> }) => {
      if (!row.original.receiptDate) return '-';
  ...
  },
  {
    ...
    cell: ({ row }: { row: Row<Receipt> }) => {
      // Sum up all receipt amounts from items array
      const totalReceiptAmount = row.original.items?.reduce((sum, item) => sum + (item.receiptAmount || 0), 0) || 0;
      return (
        <span className="font-medium">
          {totalReceiptAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </span>
      );
    },
    enableColumnFilter: true,
