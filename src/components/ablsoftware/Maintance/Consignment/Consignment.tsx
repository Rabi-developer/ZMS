  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setSbrTaxes(taxRes.data.map((t: Tax) => ({ id: t.id, name: t.taxName || t.name })));
        setShippingLines(transporterRes.data.map((t: Transporter) => ({ id: t.id, name: t.name })));
      ...
      if (bookingOrderId) {
        try {
          const consPayload = {
            ...
            item: payload.items?.[0]?.desc || '',
            qty: payload.items?.[0]?.qty || 0,
            totalAmount: payload.totalAmount || 0,
            recvAmount: payload.receivedAmount || 0,
                ...
                <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1">
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <ABLNewCustomInput
                        {...field}
                        value={fromBooking ? bookingOrderNo : field.value}
                        label="Order No"
                        type="text"
                        placeholder="Auto populated from booking order"
                        register={register}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl max-w-6xl w-full mx-4 max-h-[90vh] overflow-hidden">
                  ...
                  <tbody>
                    {bookingOrders
                      ...
                      .map((order) => (
                        ...
                        >
                          <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{order.orderNo}</td>
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{order.vehicleNo}</td>
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{order.cargoWeight}</td>
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{order.orderDate}</td>
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{order.vendor}</td>
              ...
              <div className="flex justify-end mt-6">
                <Button onClick={() => setShowOrderPopup(false)} className="bg-gray-500 hover:bg-gray-600 text-white">
                  Cancel
                </Button>
              </div>
            </div>
