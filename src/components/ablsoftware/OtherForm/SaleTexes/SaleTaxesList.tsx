...
import { getAllSaleTexes, deleteSaleTexes } from '@/apis/salestexes';
import { columns, SalesTaxType } from '@/components/ablsoftware/OtherForm/SaleTexes/columns';
...
const SalesTaxesList = () => {
  const [salesTaxes, setSalesTaxes] = useState<SalesTaxType[]>([]);
  const [loading, setLoading] = useState(false);
  const [openDelete, setOpenDelete] = useState(false);
  const [openView, setOpenView] = useState(false);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [selectedSalesTax, setSelectedSalesTax] = useState<SalesTaxType | null>(null);
  ...
  const fetchSalesTaxes = useCallback(async () => {
    try {
      setLoading(true);
      // Convert 0-based pageIndex to 1-based for API
      const apiPageIndex = pageIndex + 1;
      console.log('Fetching sales taxes with pageIndex:', pageIndex, 'apiPageIndex:', apiPageIndex, 'pageSize:', pageSize);
      
      const response: ApiResponse = await getAllSaleTexes(apiPageIndex, pageSize);
      ...
      console.error('Failed to fetch sales taxes:', error);
      toast.error('Failed to fetch sales taxes');
    } finally {
      setLoading(false);
    }
  }, [pageIndex, pageSize]);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSaleTexes(deleteId);
      setSalesTaxes(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete Sales Tax:', error);
      toast.error('Failed to delete sales tax');
    }
  };
  ...
  const handleViewOpen = (id: string) => {
    const item = salesTaxes.find(item => item.id === id);
    setSelectedSalesTax(item || null);
    setOpenView(true);
  };

  const handleViewClose = () => {
    setOpenView(false);
    setSelectedSalesTax(null);
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSalesTax.taxType}
                      </div>
                    </div>
                    {selectedSalesTax.percentage && (
                      <div className="group">
                        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                          Percentage (%)
                        </span>
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSalesTax.receivable.accountId}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSalesTax.receivable.description}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSalesTax.payable.accountId}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSalesTax.payable.description}
                      </div>
                    </div>
                  </div>
                </div>
