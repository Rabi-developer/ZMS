...
const AblNewCustomDrpdown: React.FC<CustomDropdownProps> = ({
  label,
  options,
  selectedOption,
  onChange,
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      ...
      const optionToSelect = filteredOptions[selectedIndex >= 0 ? selectedIndex : 0];
  ...
  return (
    <div className="w-full abl-custom-dropdown dropdown" ref={dropdownRef}>
      <div className="mb-4 flex items-center gap-4">
        <label
          htmlFor="dropdown"
          className={`w-32 text-lg font-medium text-gray-700 whitespace-nowrap transition-all duration-300 ${
        ...
        <div className="relative flex-1">
          <input
            ...
            disabled={disabled}
            className={`w-full h-11 px-4 py-3 rounded-t-lg text-[#6d6d6d] placeholder-gray-400 shadow-md focus:outline-none transition-all duration-300 bg-white
...
export default AblNewCustomDrpdown;
