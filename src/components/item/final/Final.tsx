...
import { createFinal, updateFinal, getAllFinal } from '@/apis/final';
...
const Final = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  React.useEffect(() => {
    if (isEdit) {
      const fetchFinal = async () => {
        const listid = window.location.pathname.split('/final').pop();
        if (listid) {
          try {
            const response = await getAllFinal();
            const foundFinal = response.data.find((item: any) => item.listid === listid);
            if (foundFinal) {
              setValue('listid', foundFinal.listid || '');
              setValue('descriptions', foundFinal.descriptions || '');
              const subDescArray =
                foundFinal.subDescription?.split('|')?.filter((s: string) => s) || [''];
              setSubDescriptions(subDescArray);
              setValue('subDescription', foundFinal.subDescription || '');
            } else {
              toast.error('Final not found');
              router.push('/final');
            }
          } catch (error) {
  ...
  const onSubmit = async (data: FinalData) => {
    try {
      if (isEdit) {
        await updateFinal(data.listid!, data);
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? 'EDIT Final' : 'ADD NEW Final'}
        </h1>
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full h-[8vh] flex justify-end gap-2 mt-3 border-t-2 border-[#e7e7e7]">
          ...
          >
            {isEdit ? 'Update Final' : 'Create Final'}
          </Button>
          <Link href="/final">
            <Button
              type="button"
