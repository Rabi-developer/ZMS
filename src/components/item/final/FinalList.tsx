...
import { getAllFinal, deleteFinal } from '@/apis/final'; 
import { columns, FinalType } from '@/components/item/final/columns';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const FinalList = () => {
  const [Final, setFinal] = React.useState<FinalType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedFinal, setSelectedFinal] = React.useState<FinalType | null>(null);
  ...
  const fetchFinal = async () => {
    try {
      setLoading(true);
      const response = await getAllFinal(pageIndex === 0 ? 1 : pageIndex, pageSize);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteFinal(deleteId);
      setFinal(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete Final:', error);
      toast.error('Failed to delete Final');
    }
  };
  ...
  const handleViewOpen = (ListId: string) => { 
    const item = Final.find(item => item.listid === ListId);
    setSelectedFinal(item || null);
    setOpenView(true);
  };
