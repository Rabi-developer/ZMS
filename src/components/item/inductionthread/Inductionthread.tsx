...
type InductionThreadFormData = z.infer<typeof inductionThreadSchema>;

const InductionThreadForm = ({ isEdit = false }: { isEdit?: boolean }) => {
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? 'EDIT INDUCTION THREAD' : 'ADD NEW INDUCTION THREAD'}
        </h1>
      </div>
...
export default InductionThreadForm;