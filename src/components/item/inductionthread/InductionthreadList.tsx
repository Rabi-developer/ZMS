...
const InductionThreadList = () => {
  const [inductionThreads, setInductionThreads] = React.useState<InductionThreadType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedInductionThread, setSelectedInductionThread] = React.useState<InductionThreadType | null>(null);
  ...
  const fetchInductionThreads = async () => {
    try {
      ...
      setInductionThreads(response.data || []);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setInductionThreads(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete InductionThread:', error);
      toast.error('Failed to delete inductionThread');
    }
  };
  ...
  const handleViewOpen = (listId: string) => { 
    const item = inductionThreads.find(item => item.listid === listId);
    setSelectedInductionThread(item || null);
    setOpenView(true);
  };

  const handleViewClose = () => {
    setOpenView(false);
    setSelectedInductionThread(null);
  };
  ...
  return (
    <div className="container bg-white rounded-md">
      <DataTable
        ...
        data={inductionThreads}
        loading={loading}
        link={'/inductionthread/create'}
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedInductionThread.listid} 
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedInductionThread.descriptions}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedInductionThread.subDescription}
                      </div>
                    </div>
                  </div>
                </div>
