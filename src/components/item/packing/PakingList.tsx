...
import { getAllPackings, deletePacking } from '@/apis/packing'; 
import { columns, PackingType } from '@/components/item/packing/columns';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const PackingList = () => {
  const [Packing, setPacking] = React.useState<PackingType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedPacking, setSelectedPacking] = React.useState<PackingType | null>(null);
  ...
  const fetchPacking = async () => {
    try {
      setLoading(true);
      const response = await getAllPackings(pageIndex === 0 ? 1 : pageIndex, pageSize);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePacking(deleteId);
      setPacking(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete Packing:', error);
  ...
  const handleViewOpen = (ListId: string) => { 
    const item = Packing.find(item => item.listid === ListId);
    setSelectedPacking(item || null);
    setOpenView(true);
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedPacking.subDescription}
                      </div>
                    </div>
                  </div>
                </div>
