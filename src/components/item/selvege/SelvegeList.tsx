...
import { getAllSelveges, deleteSelvege } from '@/apis/selvege'; 
import { columns, SelvegeType } from '@/components/item/selvege/columns';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const SelvegeList = () => {
  const [Selvege, setSelvege] = React.useState<SelvegeType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedSelvege, setSelectedSelvege] = React.useState<SelvegeType | null>(null);
  ...
  const fetchSelvege = async () => {
    try {
      setLoading(true);
      const response = await getAllSelveges(pageIndex === 0 ? 1 : pageIndex, pageSize);
      setSelvege(response.data || []);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSelvege(deleteId);
      setSelvege(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete Selvege:', error);
      toast.error('Failed to delete Selvege');
    }
  };
  ...
  const handleViewOpen = (ListId: string) => { 
    const item = Selvege.find(item => item.listid === ListId);
    setSelectedSelvege(item || null);
    setOpenView(true);
  };

  const handleViewClose = () => {
    setOpenView(false);
    setSelectedSelvege(null);
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSelvege.subDescription}
                      </div>
                    </div>
                  </div>
                </div>
