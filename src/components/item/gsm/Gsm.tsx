...
import { createGSM, updateGSM, getAllGSMs } from '@/apis/gsm';
      ...
      const fetchGSM = async () => {
        ...
        if (listid) {
          try {
            ...
            const foundGSM = response.data.find((item: any) => item.listid === listid);
            if (foundGSM) {
              ...
              toast.error('GSM not found');
              router.push('/gsm');
            }
          } catch (error) {
            console.error('Error fetching GSM:', error);
            toast.error('Failed to load GSM');
          }
        }
      };
      fetchGSM();
  ...
  const onSubmit = async (data: GSMData) => {
    try {
      if (isEdit) {
        ...
        await createGSM(data);
        toast.success('GSM created successfully!');
      }
      router.push('/gsm');
    } catch (error) {
      toast.error('An error occurred while saving the GSM');
    }
  };
