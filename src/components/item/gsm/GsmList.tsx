...
const GSMList = () => {
  ...
  const [selectedGSM, setSelectedGSM] = React.useState<GSMType | null>(null);
  ...
  const fetchGSM = async () => {
    try {
      setLoading(true);
      const response = await getAllGSMs(pageIndex === 0 ? 1 : pageIndex, pageSize);
      setGSM(response.data || []);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteGSM(deleteId);
      setGSM(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete GSM:', error);
