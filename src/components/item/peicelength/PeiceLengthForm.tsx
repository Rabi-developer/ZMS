...
const PeiceLengthSchema = z.object({
...
type PeiceLengthFormData = z.infer<typeof PeiceLengthSchema>;

const PeiceLengthForm = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<PeiceLengthFormData>({
    resolver: zodResolver(PeiceLengthSchema),
    defaultValues: {
      listid: '',
      descriptions: '',
      subDescription: '',
      ...
      const fetchPeiceLength = async () => {
        ...
        if (listid) {
          try {
            ...
            const foundPeiceLength = response.data.find((item: any) => item.listid === listid); 
            if (foundPeiceLength) {
              setValue('listid', foundPeiceLength.listid || '');
              setValue('descriptions', foundPeiceLength.descriptions || '');
              setValue('subDescription', foundPeiceLength.subDescription || '');
              setValue('useDeletedId', false);
            } else {
              toast.error('Peice Length not found');
              router.push('/peicelength');
            }
          } catch (error) {
            console.error('Error fetching Peice Length:', error);
            toast.error('Failed to load Peice Length');
          }
        }
      };
      fetchPeiceLength();
  ...
  const onSubmit = async (data: PeiceLengthFormData) => {
    try {
      if (isEdit) {
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? "EDIT PeiceLength" : "ADD NEW PeiceLength"}
        </h1>
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full h-[8vh] flex justify-end gap-2 mt-3 border-t-2 border-[#e7e7e7]">
          ...
          >
            {isEdit ? "Update PeiceLength" : "Create PeiceLength"}
          </Button>
...
export default PeiceLengthForm;