...
import { getAllSelvegeThicknesss, deleteSelvegeThickness  } from '@/apis/selvegethickness';
import { columns, SelvegeThicknessType } from '@/components/item/selvegethickness/columns';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const SelvegeThicknessList = () => {
  const [selvegeThicknesses, setSelvegeThicknesses] = React.useState<SelvegeThicknessType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedSelvegeThickness, setSelectedSelvegeThickness] = React.useState<SelvegeThicknessType | null>(null);
  ...
  const fetchSelvegeThicknesses = async () => {
    try {
      setLoading(true);
      const response = await getAllSelvegeThicknesss(pageIndex === 0 ? 1 : pageIndex, pageSize);
      setSelvegeThicknesses(response.data || []);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSelvegeThickness (deleteId);
      setSelvegeThicknesses(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete Selvege Thickness:', error);
      toast.error('Failed to delete Selvege Thickness');
    }
  };
  ...
  const handleViewOpen = (listId: string) => {
    const item = selvegeThicknesses.find(item => item.listid === listId);
    setSelectedSelvegeThickness(item || null);
    setOpenView(true);
  };

  const handleViewClose = () => {
    setOpenView(false);
    setSelectedSelvegeThickness(null);
  };
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSelvegeThickness.listid}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSelvegeThickness.descriptions}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSelvegeThickness.subDescription}
                      </div>
                    </div>
                  </div>
                </div>
...
export default SelvegeThicknessList;