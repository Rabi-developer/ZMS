...
import { createSelvegeThickness, updateSelvegeThickness, getAllSelvegeThicknesss } from '@/apis/selvegethickness';
...
const SelvegeThicknessSchema = z.object({
...
type SelvegeThicknessData = z.infer<typeof SelvegeThicknessSchema>;

const SelvegeThickness = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<SelvegeThicknessData>({
    resolver: zodResolver(SelvegeThicknessSchema),
    defaultValues: {
      listid: '',
      descriptions: '',
      subDescription: '',
      ...
      const fetchSelvegeThickness = async () => {
        ...
        if (listid) {  
          try {
            ...
            if (foundSelvegeThickness) {
              setValue('listid', foundSelvegeThickness.listid || '');
              setValue('descriptions', foundSelvegeThickness.descriptions || '');
              const subDescArray =
                foundSelvegeThickness.subDescription?.split('|')?.filter((s: string) => s) || [''];
              setSubDescriptions(subDescArray);
              setValue('subDescription', foundSelvegeThickness.subDescription || '');
            } else {
              toast.error('Selvege Thickness not found');
              router.push('/selvegeThickness');
            }
          } catch (error) {
            console.error('Error fetching Selvege Thickness:', error);
            toast.error('Failed to load Selvege Thickness');
          }
        }
      };
      fetchSelvegeThickness();
    }
  ...
  const onSubmit = async (data: SelvegeThicknessData) => {
    try {
      if (isEdit) {
        await updateSelvegeThickness(data.listid!, data);
        toast.success('Selvege Thickness updated successfully!');
      } else {
        await createSelvegeThickness(data);
        toast.success('Selvege Thickness created successfully!');
      }
      router.push('/selvegethickness');
    } catch (error) {
      toast.error('An error occurred while saving the Selvege Thickness');
    }
  };
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? 'EDIT Selvege Thickness' : 'ADD NEW Selvege Thickness'}
        </h1>
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full h-[8vh] flex justify-end gap-2 mt-3 border-t-2 border-[#e7e7e7]">
          ...
          >
            {isEdit ? 'Update Selvege Thickness' : 'Create Selvege Thickness'}
          </Button>
          <Link href="/selvegethickness">
            <Button
              type="button"
...
export default SelvegeThickness;