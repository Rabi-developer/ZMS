        ...
        if (listid) {
          try {
            ...
            if (foundSelvegeWeaves) {
              ...
              const subDescArray = foundSelvegeWeaves.subDescription?.split('|')?.filter((s: string) => s) || [''];
              setSubDescriptions(subDescArray);
              setValue('subDescription', foundSelvegeWeaves.subDescription || '');
            } else {
              toast.error('Selvege Weaves not found');
              router.push('/selvegeweaves');
            }
          } catch (error) {
            console.error('Error fetching Selvege Weaves:', error);
            toast.error('Failed to load Selvege Weaves');
          }
        }
      };
      fetchSelvegeWeaves();
  ...
  const onSubmit = async (data: SelvegeWeavesFormData) => {
    try {
      if (isEdit) {
        ...
        toast.success('Selvege Weaves updated successfully!');
      } else {
        await createSelvegeWeave(data);
        toast.success('Selvege Weaves created successfully!');
      }
      router.push('/selvegeweaves');
    } catch (error) {
      toast.error('An error occurred while saving the Selvege Weaves');
    }
  };
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full h-[8vh] flex justify-end gap-2 mt-3 border-t-2 border-[#e7e7e7]">
          ...
          >
            {isEdit ? 'Update Selvege Weaves' : 'Create Selvege Weaves'}
          </Button>
