...
import { getAllPickInsertions, deletePickInsertion } from '@/apis/pickinsertion'; 
import { columns, PickInsertionType } from '@/components/item/pickinsertion/columns';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const PickInsertionList = () => {
  const [PickInsertion, setPickInsertion] = React.useState<PickInsertionType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedPickInsertion, setSelectedPickInsertion] = React.useState<PickInsertionType | null>(null);
  ...
  const fetchPickInsertion = async () => {
    try {
      setLoading(true);
      const response = await getAllPickInsertions(pageIndex === 0 ? 1 : pageIndex, pageSize);
      setPickInsertion(response.data || []);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePickInsertion(deleteId);
      setPickInsertion(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete PickInsertion:', error);
      toast.error('Failed to delete PickInsertion');
    }
  };
  ...
  const handleViewOpen = (ListId: string) => { 
    const item = PickInsertion.find(item => item.listid === ListId);
    setSelectedPickInsertion(item || null);
    setOpenView(true);
  };

  const handleViewClose = () => {
    setOpenView(false);
    setSelectedPickInsertion(null);
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedPickInsertion.descriptions}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedPickInsertion.subDescription}
                      </div>
                    </div>
                  </div>
                </div>
...
export default PickInsertionList;
