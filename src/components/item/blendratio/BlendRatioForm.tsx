...
type BlendRatioFormData = z.infer<typeof blendratioSchema>;

const BlendRatioForm = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<BlendRatioFormData>({
    resolver: zodResolver(blendratioSchema),
    defaultValues: {
      listid: '',
      descriptions: '',
      subDescription: '',
      ...
      const fetchBlendRatio = async () => {
        ...
        if (listid) {
          try {
            ...
            const foundblendratio = response.data.find((desc: any) => desc.listid === listid);
            if (foundblendratio) {
            ...
            toast.error('Failed to load Blend Ratio');
          }
        }
      };
      fetchBlendRatio();
  ...
  const onSubmit = async (data: BlendRatioFormData) => {
    try {
      if (isEdit) {
        await updateBlendRatio(data.listid!, data);
        toast.success('BlendRatio updated successfully!');
      } else {
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? "EDIT BLEND RATIO" : "ADD NEW BLEND RATIO"}
        </h1>
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full h-[8vh] flex justify-end gap-2 mt-3 border-t-2 border-[#e7e7e7]">
          ...
          >
            {isEdit ? "Update BlendRatio" : "Create BlendRatio"}
          </Button>
          <Link href="/bendRatio">
            <Button
              type="button"
