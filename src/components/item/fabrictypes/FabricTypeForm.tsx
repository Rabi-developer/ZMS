...
import { createFabricTypes, updateFabricTypes, getAllFabricTypess } from '@/apis/fabrictypes';
...
const FabricTypeForm = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  React.useEffect(() => {
    if (isEdit) {
      const fetchFabricType = async () => {
        const listid = window.location.pathname.split('/fabrictype').pop();
        if (listid) {
          try {
            const response = await getAllFabricTypess();
            const foundFabricType = response.data.find((item: any) => item.listid === listid);
            if (foundFabricType) {
              setValue('listid', foundFabricType.listid || '');
              setValue('descriptions', foundFabricType.descriptions || '');
              const subDescArray =
                foundFabricType.subDescription?.split('|')?.filter((s: string) => s) || [''];
              setSubDescriptions(subDescArray);
              setValue('subDescription', foundFabricType.subDescription || '');
            } else {
              toast.error('Fabric Type not found');
              router.push('/fabrictypes');
            }
          } catch (error) {
            console.error('Error fetching Fabric Type:', error);
            toast.error('Failed to load Fabric Type');
          }
        }
      };
      fetchFabricType();
  ...
  const onSubmit = async (data: FabricTypeFormData) => {
    try {
      if (isEdit) {
        await updateFabricTypes(data.listid!, data);
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? 'EDIT FabricType' : 'ADD NEW FabricType'}
        </h1>
      </div>
