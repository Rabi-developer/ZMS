...
import { createStuff, updateStuff, getAllStuffs } from '@/apis/stuff';
...
type StuffFormData = z.infer<typeof stuffSchema>;

const StuffForm = ({ isEdit = false }: { isEdit?: boolean }) => {
      ...
      const fetchStuff = async () => {
        ...
        if (listid) {
          try {
            ...
            const foundStuff = response.data.find((item: any) => item.listid === listid);
            if (foundStuff) {
              setValue('listid', foundStuff.listid || '');
              setValue('descriptions', foundStuff.descriptions || '');
              const subDescArray = foundStuff.subDescription?.split('|')?.filter((s: string) => s) || [''];
              setSubDescriptions(subDescArray);
              setValue('subDescription', foundStuff.subDescription || '');
            } else {
              toast.error('Stuff not found');
              router.push('/stuff');
            }
          } catch (error) {
  ...
  const onSubmit = async (data: StuffFormData) => {
    try {
      if (isEdit) {
        await updateStuff(data.listid!, data);
      ...
      toast.error('An error occurred while saving the stuff');
    }
  };
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? 'EDIT STUFF' : 'ADD NEW STUFF'}
        </h1>
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full h-[8vh] flex justify-end gap-2 mt-3 border-t-2 border-[#e7e7e7]">
          ...
          >
            {isEdit ? 'Update Stuff' : 'Create Stuff'}
          </Button>
          <Link href="/stuff">
            <Button
              type="button"
