...
import { createSelvegeWidth, updateSelvegeWidth, getAllSelvegeWidths } from '@/apis/selvegewidth';
...
const SelvegeWidthSchema = z.object({
...
type SelvegeWidthFormData = z.infer<typeof SelvegeWidthSchema>;

const SelvegeWidthForm = ({ isEdit = false }: { isEdit?: boolean }) => {
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<SelvegeWidthFormData>({
    resolver: zodResolver(SelvegeWidthSchema),
    defaultValues: {
      listid: '',
      descriptions: '',
      subDescription: '',
  ...
  React.useEffect(() => {
    if (isEdit) {
      const fetchSelvegeWidth = async () => {
        const listid = window.location.pathname.split('/selvegewidth').pop();
        if (listid) {
          try {
            const response = await getAllSelvegeWidths();
            const foundSelvegeWidth = response.data.find((item: any) => item.listid === listid);
            if (foundSelvegeWidth) {
              setValue('listid', foundSelvegeWidth.listid || '');
              setValue('descriptions', foundSelvegeWidth.descriptions || '');
              const subDescArray = foundSelvegeWidth.subDescription?.split('|')?.filter((s: string) => s) || [''];
              setSubDescriptions(subDescArray);
              setValue('subDescription', foundSelvegeWidth.subDescription || '');
            } else {
              toast.error('Selvege Width not found');
              router.push('/selvegewidth');
            }
          } catch (error) {
            console.error('Error fetching Selvege Width:', error);
            toast.error('Failed to load Selvege Width');
          }
        }
      };
      fetchSelvegeWidth();
  ...
  const onSubmit = async (data: SelvegeWidthFormData) => {
    try {
      if (isEdit) {
        await updateSelvegeWidth(data.listid!, data);
        toast.success('Selvege Width updated successfully!');
      } else {
        await createSelvegeWidth(data);
        toast.success('Selvege Width created successfully!');
      }
      router.push('/selvegewidth');
    } catch (error) {
      toast.error('An error occurred while saving the Selvege Width');
    }
  };
    ...
    <div className="container mx-auto bg-white shadow-lg rounded dark:bg-[#030630]">
      <div className="w-full bg-[#06b6d4] h-[7vh] rounded">
        <h1 className="text-[23px] font-mono ml-10 mt-8 pt-3 text-white flex gap-2">
          <MdAddBusiness />
          {isEdit ? 'EDIT Selvege Width' : 'ADD NEW Selvege Width'}
        </h1>
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full h-[8vh] flex justify-end gap-2 mt-3 border-t-2 border-[#e7e7e7]">
          ...
          >
            {isEdit ? 'Update Selvege Width' : 'Create Selvege Width'}
          </Button>
          <Link href="/selvegewidth">
            <Button
              type="button"
...
export default SelvegeWidthForm;