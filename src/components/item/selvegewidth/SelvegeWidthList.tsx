...
import { getAllSelvegeWidths, deleteSelvegeWidth } from '@/apis/selvegewidth'; 
import { columns, SelvegeWidthType } from '@/components/item/selvegewidth/columns';
import { DataTable } from '@/components/ui/table';
import DeleteConfirmModel from '@/components/ui/DeleteConfirmModel';
import { toast } from 'react-toastify';

const SelvegeWidthList = () => {
  const [SelvegeWidth, setSelvegeWidth] = React.useState<SelvegeWidthType[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);
  const [openView, setOpenView] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [selectedSelvegeWidth, setSelectedSelvegeWidth] = React.useState<SelvegeWidthType | null>(null);
  ...
  const fetchSelvegeWidth = async () => {
    try {
      setLoading(true);
      const response = await getAllSelvegeWidths(pageIndex === 0 ? 1 : pageIndex, pageSize);
      setSelvegeWidth(response.data || []);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSelvegeWidth(deleteId);
      setSelvegeWidth(prev => prev.filter(item => item.id !== deleteId));
      setOpenDelete(false);
      setDeleteId(null);
      toast.success("Deleted Successfully");
    } catch (error) {
      console.error('Failed to delete SelvegeWidth:', error);
      toast.error('Failed to delete SelvegeWidth');
    }
  };
  ...
  const handleViewOpen = (ListId: string) => { 
    const item = SelvegeWidth.find(item => item.listid === ListId);
    setSelectedSelvegeWidth(item || null);
    setOpenView(true);
  };

  const handleViewClose = () => {
    setOpenView(false);
    setSelectedSelvegeWidth(null);
  };
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="group">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSelvegeWidth.descriptions}
                      </div>
                    </div>
                    <div className="group">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1 transition-colors group-hover:text-cyan-600">
                      ...
                      <div className="bg-white rounded-lg px-4 py-2 border border-gray-200 shadow-sm text-gray-800 text-lg font-medium group-hover:border-cyan-300 transition-all duration-200">
                        {selectedSelvegeWidth.subDescription}
                      </div>
                    </div>
                  </div>
                </div>
...
export default SelvegeWidthList;
